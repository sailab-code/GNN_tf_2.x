...
class GraphObject:
    ...
    def __init__(self, arcs, nodes, targets, problem_based='n', set_mask=None, output_mask=None,
                 NodeGraph=None, ArcNode=None, node_aggregation="average"):
        """ CONSTRUCTOR METHOD
        ...
        :param node_aggregation: (str) in ['average', 'sum' or 'normalized']. How graph-based problem are faced.
        """
    ...
    # -----------------------------------------------------------------------------------------------------------------
    def buildArcNode(self, node_aggregation):
    ...
    ## SETTERS ########################################################################################################
    def setArcNode(self, node_aggregation) -> None:
    ...
    ## STATIC METHODS #################################################################################################
    @staticmethod
    def load(graph_folder_path, problem_based, *, node_aggregation="average") -> object:
    ...
    # -----------------------------------------------------------------------------------------------------------------
    @staticmethod
    def save(graph_folder_path, g, *, save_set_mask=False, format='%.10g') -> None:
    ...
    # -----------------------------------------------------------------------------------------------------------------
    @staticmethod
    def merge(glist, node_aggregation) -> object:
        ...
        nodes, arcs, targets, setmask, outmask = zip(*[(i.getNodes(), i.getArcs(), i.getTargets(),
                                                        i.getSetMask(), i.getOutputMask()) for i in glist])
