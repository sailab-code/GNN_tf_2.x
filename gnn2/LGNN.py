...
from GNN.GNN import GNNnodeBased, GNNgraphBased, GNNedgeBased
from GNN.GNN_BaseClass import BaseClass
from GNN.graph_class import GraphObject, GraphTensor


class LGNN(BaseClass):
    ## CONSTRUCTORS METHODS ###########################################################################################
    def __init__(self,
                 gnns: list[GNNnodeBased, GNNedgeBased, GNNgraphBased],
                 get_state: bool,
                 get_output: bool,
        ...
        :param get_state: (bool) if True node_state are propagated through GNNs layers.
        :param get_output: (bool) if True gnn_outputs on nodes/arcs are propagated through GNNs layers.
    ...
    def copy(self, *, path_writer: str = '', namespace: str = '', copy_weights: bool = True) -> 'LGNN':
        ...
        if not namespace: namespace = 'LGNN'
        ...
        return self.__class__(gnns=[i.copy(copy_weights=copy_weights) for i in self.gnns],
                              ...
                              extra_metrics_arguments=self.mt_args, path_writer=path_writer, namespace=namespace)
    ...
    # -----------------------------------------------------------------------------------------------------------------
    @classmethod
    def load(self, path: str, path_writer: Optional[str] = None, namespace: str = 'LGNN',
    ...
    def __call__(self, g: Union[GraphObject, GraphTensor]) -> tf.Tensor:
        """ Return ONLY the LGNN output for graph g of type GraphObject/GraphTensor """
        out = self.Loop(g, training=False)[-1]
        return out[-1]

    # -----------------------------------------------------------------------------------------------------------------
    def predict(self, g: Union[GraphObject, GraphTensor], idx: Union[int, list[int], range, str] = -1) -> Union[tf.Tensor, list[tf.Tensor]]:
        ...
        if isinstance(idx, int):
            ...
            raise ValueError('param <idx> must be 1.int; 2.list of ordered ints in range(self.LAYERS); 3. str "all"')
    ...
    ## LOOP METHODS ###################################################################################################
    def update_graph(self, g: GraphTensor, state: Union[tf.Tensor, array], output: Union[tf.Tensor, array]) -> GraphObject:
        """
    ...
    # -----------------------------------------------------------------------------------------------------------------
    def Loop(self, g: Union[GraphObject, GraphTensor], *, training: bool = False) -> tuple[list[tf.Tensor], tf.Tensor, list[tf.Tensor]]:
    ...
    def train(self, gTr: Union[GraphObject, GraphTensor, list[GraphObject, GraphTensor]], epochs: int,
        ...
        if (self.training_mode is not None) and (self.training_mode != training_mode):
            raise ValueError
        ...
        if training_mode == 'serial':
            ...
            for idx, gnn in enumerate(self.gnns):
                ...
                if gVa:
                    _, sVa, oVa = zip(*[super(GNNgraphBased, gnn).Loop(i) if isinstance(gnn, GNNgraphBased) else gnn.Loop(i) for i in gVa1])
